...
export type StringValidatorType = (opts?: StringValidatorOptions) => PropertyDecorator;
...
export function StringValidator(options: StringValidatorOptions = {}) {
  ...
  // 1) 값 가공(transform)
  if (options.trim) {
    decorators.push(Transform(({ value }) => (typeof value === 'string' ? value.trim() : value)));
  }
  ...
  decorators.push(
    IsString({
      ...
      message:
        options.message ??
        (eachOption ? '$property each elements must be a string' : '$property must be a string'),
    }),
  );

  // 4) 길이 제한
  if (typeof options.minLength === 'number' || typeof options.maxLength === 'number') {
    decorators.push(
      Length(options.minLength ?? 0, options.maxLength ?? Number.MAX_SAFE_INTEGER, {
        each: eachOption,
        message:
          options.message ??
          (eachOption
